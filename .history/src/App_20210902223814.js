...
function App() {
  ...
  useEffect(() => {
    timerRef.current = setTimeout(() => {
      nextQuestion();
      if (currentQuestionNumber === quiz.length - 1) {showResultPage();clearTimeout(timerRef.current)}
    }, 10000);

    if (count > 0) countdown.current = setInterval(() => {
      setCount((count) => count-1);
    }, 1000);

    return () => {
      clearTimeout(timerRef.current); clearInterval(countdown.current);
    };
