...
function App() {
  ...
  setClickedChoice(current_answer)
  ...
  function checkAnswer(e) {
    ...
    let current_choices = [...quiz[currentQuestionNumber].choices];
    let choice = e.target.value;

    if (choice === currentAnswer) {
      correctSound.play();
      setScore(score + 10);
      //e.target.className ="btn correct py-3 w-100 fs-3 mx-md-5 rounded-pill text-dark opacity-100";
    } else {
      wrongSound.play();
      //e.target.className ="btn wrong py-3 w-100 fs-3 mx-md-5 rounded-pill text-dark opacity-100";
    }
    ...
    setClickedChoice(current_answerchoice)
    setCurrentChoices(choice);
    setCurrentChoices(current_choices);
