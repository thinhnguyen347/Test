...
function App() {
  ...
  function checkAnswer(e) {
    ...
    const wrongSound = new Howl({ src: correct });

    let current_answer = quiz[currentQuestionNumber].ans;
    let choice = e.target.textContent;

    if (choice === current_answer) { setScore(score + 10); correctSound.play() };
    ...
    <div className="App container px-md-2 px-lg-5 w-100 d-flex align-items-center">
      ...
      <div className={`container-fluid p-0 ${questionPage}`}>
        ...
        <div className='container w-100 d-flex justify-content-center my-2'>
          <button type='button' className='btn btn-primary px-4 my-5 fs-5'>{nextBtn}</button>
        </div>
      </div>
