...
function App() {
  ...
  useEffect(() => {
    ...
    return ()=> {clearTimeout(timerRef.current); clearInterval(countdown)}
  });

  // useEffect(() => {
// if (count===0){clearInterval(countdown.current); showResultPage()}
  //  countdown.current = setInterval(() => {
  //     setCount((count) => count-1); nextQuestion();
  //   }, 1000);
