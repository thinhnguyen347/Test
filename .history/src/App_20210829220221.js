...
function App() {
  ...
  const [hideNextBtn, setHideNextBtnDisable] = useState(true);
  const [finishBtnDisable, setFinishBtnDisable] = useState(true);
  const [choiceBtnDisable, setChoiceBtnDisable] = useState(false);
  const [score, setScore] = useState(0);
  const current_question = quiz[currentQuestionNumber];
  const question_number = useRef(currentQuestionNumber);
    ...
    <div className="App container px-md-2 px-lg-5 w-100 d-flex align-items-center">
      ...
      <div className={`container-fluid p-0 ${questionPage}`}>
        ...
        <div className='container-fluid d-flex justify-content-around my-5 px-lg-5'>
          <button type='button' className='btn btn-primary px-3 px-sm-5 fs-5' onClick={nextQuestion} disabled={nextBtnDisable}>&nbsp;Next&nbsp;</button>
          <button type='button' className='btn btn-danger px-3 px-sm-5 fs-5' onClick={showResultPage} disabled={finishBtnDisable}>Finish</button>
        </div>
      </div>
