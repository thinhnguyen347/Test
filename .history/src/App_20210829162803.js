...
function App() {
  ...
  const [nextBtn, setNextBtn] = useState('d-block');
  const [finishBtn, setFinishBtn] = useState('d-none');
  ...
  function RenderQuestion({ question, choices }) {
    return (<>
      <div className='App-question-area d-flex align-items-center justify-content-center mb-5 px-lg-5'>
  ...
  function nextQuestion() {
    ...
    if (currentQuestionNumber < quiz.length) { setCurrentQuestionNumber(currentQuestionNumber + 1) } else { setNextBtn("d-none") };

    if (currentQuestionNumber === quiz.length) setFinishBtn("d-block");
    ...
    <div className="App container px-md-2 px-lg-5 w-100 d-flex align-items-center">
      ...
      <div className={`container-fluid p-0 ${questionPage}`}>
        <div className={'container-fluid p-0'}>
          <div className="container-fluid pt-5 pb-2 d-flex justify-content-between fs-5">
        ...
        <div className='container w-100 d-flex justify-content-center my-2'>
          <button type='button' className={`${nextBtn} btn btn-primary px-4 my-5 fs-5`} onClick={nextQuestion}>Next</button>
          <button type='button' className={`${finishBtn} btn btn-danger px-4 my-5 fs-5`} onClick={showResultPage}>Finish</button>
